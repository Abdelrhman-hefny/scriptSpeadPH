...
function sampleRowAverage(doc, yPx, sampleCount, ignoreThreshold) {
  ...
  return [
    Math.round(sumR / kept),
    Math.round(sumG / kept),
    Math.round(sumB / kept),
  ];
}
...
function applyGradientAndStroke(colorTop, colorBottom) {
  ...
  stopBottom.putObject(
    charIDToTypeID("Clr "),
    charIDToTypeID("RGBC"),
    stopBottomClr
  );
  stopBottom.putEnumerated(
    charIDToTypeID("Type"),
    charIDToTypeID("Clry"),
    charIDToTypeID("UsrS")
  );
...
function main() {
  ...
  try {
    ...
    var bottomRGB = sampleRowAverage(
      doc,
      bottomY,
      samplesPerRow,
      ignoreThreshold

...
def merge_export_splits(
    ...
    if not merge_splits:
        ...
        for split_file, split_path_list in split_files.items():
            # Create a mapping from split file name to full split path
            name_to_split_path = {Path(p).name: p for p in split_path_list}

            split_export_targets: dict[str, ie.ExportTarget] = {}
            for export_target in export_targets:
                name = export_target.original_path.name
                if name in name_to_split_path:
                    # Use the real full split path as the key
                    split_export_targets[name_to_split_path[name]] = export_target

            # Verify we found all of them.
            if len(split_export_targets) != len(split_path_list):
                logger.error(f"Could not find all split images for {split_file}. Skipping.")
                continue

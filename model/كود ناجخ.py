...
try:
    ...
    IOU_THRESHOLD = 0.7          # IOU لـ YOLO NMS (قيمة عالية للسماح بالتداخل)
    MERGE_IOU_THRESHOLD = 0.5    # عتبة معتدلة للدمج بين الشرائح
    SLICE_OVERLAP = 300
    SLICE_HEIGHT = 4000
    ...
    # 💡 القيمة الجديدة: توسيع الصندوق المحيط لضمان تغطية الفجوات الضيقة
    BOX_EXPANSION_PIXELS = 5 # 💡 تم تخفيضها إلى 5 بكسل للوصول إلى المستوى المطلوب
    ...
    def merge_and_clean_boxes(boxes_raw, scores_raw):
        if not boxes_raw:
            return [], []
        # فرز الصناديق حسب الثقة تنازلياً للحفاظ على الأفضل
        ...
        for i in range(len(boxes)):
            ...
            for j in picked_indices:
                # استخدام MERGE_IOU_THRESHOLD للدمج بين الاكتشافات المكررة عبر الشرائح
                if box_iou(boxes[i], final_boxes[j]) > MERGE_IOU_THRESHOLD:
                    # دمج الصندوقين المتطابقين في صندوق واحد أكبر (لتغطية كاملة)
                    fb = final_boxes[j]
        ...
        # إزالة الصناديق المحتواة داخل أخرى (التنظيف النهائي)
        to_remove = set()
    ...
    # 💡 تم حذف دالة get_bubble_polygon
    ...
    for idx, img_file in enumerate(image_files, start=1):
        ...
        for box in merged_boxes:
            # 💡 يتم تطبيق التوسيع البسيط هنا (5 بكسل)
            x1, y1, x2, y2 = box
            
            # تطبيق التوسيع مع التأكد من عدم تجاوز حدود الصورة
            ...
            # تأكد من أن الصندوق يحتوي على نص
            if has_text(image, box_expanded):
                cx, cy = get_box_center(box_expanded)
                
                # 💡 توليد البوليغون من الصندوق المحيط الموسع (4 نقاط فقط)
                polygon = [
...
from datetime import datetime
import traceback
import math # تمت الإضافة للاستخدام المستقبلي المحتمل (مثل عمليات رياضية)
...
try:
    # ===== المسارات والإعدادات الأساسية (لا تغيير) =====
    cfg_path = r"C:\Users\abdoh\Downloads\testScript\config\temp-title.json"
    with open(cfg_path, encoding="utf-8") as f:
        cfg = json.load(f)
    ...
    # ===== إعدادات الكشف (التعديلات الرئيسية) =====
    # 💡 التعديل 1: خفض عتبة الثقة لزيادة حساسية YOLO (الضرورية للفقاعات الباهتة/الشفافة).
    CONFIDENCE_THRESHOLD = 0.01  # تم تخفيضها من 0.04 إلى 0.01
    
    IOU_THRESHOLD = 0.7
    MERGE_IOU_THRESHOLD = 0.5
    SLICE_OVERLAP = 300
    SLICE_HEIGHT = 4000
    # 💡 التعديل 2: خفض الحد الأدنى للمساحة لتمكين اكتشاف الفقاعات الصغيرة جدًا أو المشوهة.
    MIN_BUBBLE_AREA = 1000  # تم تخفيضها من 2000 إلى 1000
    CONTAINMENT_THRESHOLD = 0.95
    MIN_DIM_THRESHOLD = 50   # تم تخفيضها من 100 إلى 50
    YOLO_IMG_SIZE = 1280
    BOX_EXPANSION_PIXELS = 10 # زيادة التوسيع قليلاً للمساعدة في OCR

    # ===== دوال مساعدة =====
    
    # 🌟 التعديل 3: دالة معالجة مسبقة مُحسَّنة للفقاعات الباهتة/الشفافة
    def preprocess_image(img):
        # 1. تحسين التباين والسطوع العام (Lab Space)
        lab = cv2.cvtColor(img, cv2.COLOR_BGR2LAB)
        l, a, b = cv2.split(lab)
        clahe_l = cv2.createCLAHE(clipLimit=3.0, tileGridSize=(8, 8)) # زيادة clipLimit
        l = clahe_l.apply(l)
        lab_enhanced = cv2.merge([l, a, b])
        enhanced_lab = cv2.cvtColor(lab_enhanced, cv2.COLOR_LAB2BGR)
        ...
        # 3. دمج التحسينين (موزون)
        final = cv2.addWeighted(enhanced_lab, 0.7, enhanced_hsv, 0.3, 0)
        
        # 4. (إضافة جديدة) تحويل بسيط للأبيض والأسود لإبراز الحواف بشكل إضافي
        gray = cv2.cvtColor(final, cv2.COLOR_BGR2GRAY)
        
        # تحويل للصورة الملونة لاستخدامها في نموذج YOLO
        final_bgr = cv2.cvtColor(gray, cv2.COLOR_GRAY2BGR)
        
        # استخدام دمج خفيف بين الصورة الأصلية المحسنة وصورة الحواف
        final = cv2.addWeighted(final, 0.8, final_bgr, 0.2, 0)
        
        return final
    
    # ... (بقية الدوال المساعدة: smart_slice_image, box_iou, is_contained, merge_and_clean_boxes, has_text, get_box_center كما هي) ...
    ...
    for idx, img_file in enumerate(image_files, start=1):
        ...
        # سيتم تطبيق المعالجة المسبقة المُحسَّنة هنا
        enhanced = preprocess_image(image)
        all_boxes, all_scores = [], []
        h, w = image.shape[:2]
        ...
        slices = smart_slice_image(enhanced, SLICE_HEIGHT, SLICE_OVERLAP, delta=200)
        for slice_img, offset_y in slices:
            # استخدام CONFIDENCE_THRESHOLD الجديد (0.01)
            results = model(
                slice_img, imgsz=YOLO_IMG_SIZE, conf=CONFIDENCE_THRESHOLD, iou=IOU_THRESHOLD, verbose=False
            )
            for r in results:
    ...
    # ===== حفظ النتيجة وتشغيل Photoshop (لا تغيير) =====
    os.makedirs(os.path.dirname(output_path), exist_ok=True)
    with open(output_path, "w", encoding="utf-8") as f:
        json.dump(all_bubbles, f, indent=2, ensure_ascii=False)
    logger.info(f"💾 Saved all bubble data to: {output_path}")
